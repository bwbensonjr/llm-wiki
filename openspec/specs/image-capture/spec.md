...
Let the curator opt a source into image localization at capture time so that
meaningful figures are preserved alongside the raw twin and can enter the
curated `wiki/` layer. When enabled for the web/Jina route, eligible images are
downloaded into the immutable `raw/assets/` layer with the twin's links
rewritten to local copies; decorative noise is filtered mechanically. During the
author interview the LLM distills figures into summary prose, and only must-see
figures are lazily promoted to `wiki/assets/` for publishing. Default capture
